...
for i = 1:number_of_Dobs
    ...
    E_mid_baseline(i) = 1./(Dob{i}.BaselineParams.cf2*Dob{i}.BaselineParams.C12);
    ...
    Q_exp_normalized(i,:) = Dob{i}.Q_exp/Dob{i}.Q_exp(4);
    Q_sim_normalized(i,:) = Dob{i}.Q_sim/Dob{i}.Q_exp(4);
    
    Q_exp(i,:) = Dob{i}.Q_exp;
    Q_sim(i,:) = Dob{i}.Q_sim;
    
    plot_resistances(Dob{i});
...
for i = 1:number_of_Dobs
    ...
    Elastance_epi(i,:) = Dob{i}.Elastance_epi*E_mid_baseline(i)/E_mid_control;
    Elastance_mid(i,:) = Dob{i}.Elastance_mid*E_mid_baseline(i)/E_mid_control;
    Elastance_endo(i,:) = Dob{i}.Elastance_endo*E_mid_baseline(i)/E_mid_control;
    
    R_endo(i,:) = Dob{i}.RAendo/R_mid_control; 
    R_mid(i,:) = Dob{i}.RAmid/R_mid_control;
    R_epi(i,:) = Dob{i}.RAepi/R_mid_control;
