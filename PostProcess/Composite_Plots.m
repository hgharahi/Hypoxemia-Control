...
for i = 1:number_of_controls
    ...
    E_mid_baseline(i) = 1./(Control{i}.BaselineParams.cf2*Control{i}.BaselineParams.C12);
    R_mid_baseline(i) = (Control{i}.RAmid(4));
    ...
    Q_exp_normalized(i,:) = Control{i}.Q_exp/Control{i}.Q_exp(4);
    Q_sim_normalized(i,:) = Control{i}.Q_sim/Control{i}.Q_exp(4);
    
    Q_exp(i,:) = Control{i}.Q_exp;
    Q_sim(i,:) = Control{i}.Q_sim;
    
    plot_resistances(Control{i});
    
end

E_mid_control = mean(E_mid_baseline)
R_mid_control = mean(R_mid_baseline)


for i=1:number_of_controls
    
    Elastance_epi(i,:) = Control{i}.Elastance_epi * E_mid_baseline(i)/E_mid_control;
    Elastance_mid(i,:) = Control{i}.Elastance_mid * E_mid_baseline(i)/E_mid_control;
    Elastance_endo(i,:) = Control{i}.Elastance_endo * E_mid_baseline(i)/E_mid_control;
    
    
    R_endo(i,:) = Control{i}.RAendo/R_mid_control;
    R_mid(i,:) = Control{i}.RAmid/R_mid_control;
    R_epi(i,:) = Control{i}.RAepi/R_mid_control;
end
...
ylabel('Pzf (mmHg)','interpreter','latex','fontsize',16);
legend(p,{'Model','Experiment',['Fitted Curve (Model)',' r^2=',num2str(r1(1,2)^2,'%0.2f'),' p=',num2str(p1(1,2),'%0.4f')],['Fitted Curve (Experiment)',' r^2=',...
    num2str(r2(1,2)^2,'%0.2f'),' p=',num2str(p2(1,2),'%0.4f')]},'Location','NorthWest','FontSize',8);
ylim([0 60.0]);
